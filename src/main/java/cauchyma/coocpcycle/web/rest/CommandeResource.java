...
import cauchyma.coocpcycle.service.CommandeService;
...
/**
 * REST controller for managing {@link cauchyma.coocpcycle.domain.Commande}.
 */
@RestController
@RequestMapping("/api")
public class CommandeResource {
    ...
    public List<CommandeDTO> getAllCommandes(@RequestParam(required = false) String filter) {
        if ("produit-is-null".equals(filter)) {
            log.debug("REST request to get all Commandes where produit is null");
            return commandeService.findAllWhereProduitIsNull();
        }
